...
import { getFunctionNameWarnings, sanitizeFunctionName } from "@/lib/function-name"

export const useFunctionNameWarning = (setValue: (value: string) => void) => {
  const [lastWarning, setLastWarning] = useState<string | null>(null)

  const showWarning = (value: string) => {
    const warnings = getFunctionNameWarnings(value)
  ...
  const handleChange = (value: string) => {
    showWarning(value)
    setValue(sanitizeFunctionName(value))
  }
