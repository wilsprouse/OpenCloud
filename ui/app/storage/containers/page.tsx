...
export default function ContainerRegistry() {
  ...
  const checkServiceStatus = async () => {
    try {
      const res = await client.get<{ service: string; enabled: boolean }>("/get-service-status?service=Container Storage")
  ...
  const handleEnableService = async () => {
    ...
    try {
      await client.post("/enable-service", { service: "Container Storage" })
